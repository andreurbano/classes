...
const
  sSQLMODULOS = ' SELECT U.*, '                       +
                ...
                '        modulo M '                   +
                ' WHERE  U.ID_Usuario = :iID_Usuario' +
                '   AND  U.ID_Modulo  = M.ID_Modulo ' +
                '   AND  M.ID_Setor   = :iID_Setor'   +
                '   AND  M.ID_Grupo   = :iID_Grupo'   +
                ' ORDER BY M.Descricao ';

 sSQLGRUPOS   = ' SELECT DISTINCT '                     +
                '          M.ID_Grupo, '                +
                '          M.ID_Setor, '                +
                '          G.Descricao '                +
                ' FROM     usuario_modulo U, '          +
...
begin
  inherited;
  //
  sqlModulo.CommandText := sSQLMODULOS;
  sqlGrupo.CommandText  := sSQLGRUPOS;
  sqlSetor.CommandText  := sSQLSETORES;
  //
  cdsSetor.Active := False;
  sqlSetor.ParamByName('iID_Usuario').Value := iSistemaUsuarioMatric;
...
begin
 ...
 sqlGrupo.ParamByName('iID_Usuario').Value := iSistemaUsuarioMatric;
 sqlGrupo.ParamByName('iID_Setor').Value   := cdsSetorID_Setor.AsInteger;
...
begin
 ...
 //            
 cdsModulo.Active := False;
 sqlModulo.ParamByName('iID_Usuario').Value := iSistemaUsuarioMatric;
 sqlModulo.ParamByName('iID_Setor').Value   := cdsSetorID_Setor.AsInteger;
 sqlModulo.ParamByName('iID_Grupo').Value   := cdsGrupoID_Grupo.AsInteger;
