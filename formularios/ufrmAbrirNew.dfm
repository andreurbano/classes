inherited frmAbrirNew: TfrmAbrirNew
  Left = 521
  Top = 173
  AlphaBlend = True
  Caption = 'Abrir...'
  ClientHeight = 265
  ClientWidth = 290
  TransparentColor = True
  OnCreate = FormCreate
  ExplicitWidth = 298
  ExplicitHeight = 292
  PixelsPerInch = 96
  TextHeight = 13
  object lblSelecioneSetor: TLabel
    Left = 10
    Top = 4
    Width = 90
    Height = 13
    Caption = 'Selecione um setor'
  end
  object lblSelecioneModulo: TLabel
    Left = 10
    Top = 45
    Width = 154
    Height = 13
    Caption = 'Selecione um m'#243'dulo do sistema'
  end
  object cbxSetor: TDBLookupComboBox
    Left = 10
    Top = 18
    Width = 272
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    KeyField = 'DESCRICAO'
    ListField = 'DESCRICAO'
    ListSource = dtsSetor
    TabOrder = 0
    OnClick = cbxSetorClick
  end
  object cbxGrupo: TDBLookupComboBox
    Left = 10
    Top = 59
    Width = 272
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    KeyField = 'ID_GRUPO'
    ListField = 'DESCRICAO'
    ListSource = dtsGrupo
    TabOrder = 1
    OnClick = cbxGrupoClick
  end
  object lbxAbrirModulo: TDBLookupListBox
    Left = 10
    Top = 84
    Width = 272
    Height = 134
    Anchors = [akLeft, akTop, akRight, akBottom]
    KeyField = 'ID_Modulo'
    ListField = 'Descricao'
    ListSource = dtsModulo
    TabOrder = 2
    OnDblClick = lbxAbrirModuloDblClick
  end
  object btnAbrir: TBitBtn
    Left = 97
    Top = 228
    Width = 97
    Height = 28
    Anchors = [akBottom]
    Caption = '&Abrir...'
    Default = True
    ModalResult = 1
    TabOrder = 3
    OnClick = BitBtn1Click
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
  end
  object cdsGrupo: TClientDataSetUrb
    Aggregates = <>
    Params = <>
    ProviderName = 'dspGrupo'
    Left = 77
    Top = 113
    object cdsGrupoID_Grupo: TIntegerField
      FieldName = 'ID_Grupo'
      Required = True
    end
    object cdsGrupoID_Setor: TIntegerField
      FieldName = 'ID_Setor'
      Required = True
    end
    object cdsGrupoDescricao: TStringField
      FieldName = 'Descricao'
      Required = True
      Size = 30
    end
  end
  object dtsGrupo: TDataSourceUrb
    DataSet = cdsGrupo
    Left = 105
    Top = 113
  end
  object cdsModulo: TClientDataSetUrb
    Aggregates = <>
    Params = <>
    ProviderName = 'dspModulo'
    Left = 77
    Top = 141
    object cdsModuloID_Usuario_Modulo: TIntegerField
      FieldName = 'ID_Usuario_Modulo'
      Required = True
    end
    object cdsModuloID_Usuario: TIntegerField
      FieldName = 'ID_Usuario'
      Required = True
    end
    object cdsModuloID_Modulo: TIntegerField
      FieldName = 'ID_Modulo'
      Required = True
    end
    object cdsModuloID_Setor: TIntegerField
      FieldName = 'ID_Setor'
      Required = True
    end
    object cdsModuloID_Grupo: TIntegerField
      FieldName = 'ID_Grupo'
      Required = True
    end
    object cdsModuloDescricao: TStringField
      DisplayWidth = 50
      FieldName = 'Descricao'
      Required = True
      Size = 50
    end
    object cdsModuloNome_Form: TStringField
      FieldName = 'Nome_Form'
      Required = True
      Size = 40
    end
  end
  object dtsModulo: TDataSourceUrb
    DataSet = cdsModulo
    Left = 105
    Top = 141
  end
  object cdsSetor: TClientDataSetUrb
    Aggregates = <>
    Params = <>
    ProviderName = 'dspSetor'
    Left = 77
    Top = 85
    object cdsSetorID_Setor: TIntegerField
      FieldName = 'ID_Setor'
      Required = True
    end
    object cdsSetorDescricao: TStringField
      FieldName = 'Descricao'
      Required = True
    end
  end
  object dtsSetor: TDataSourceUrb
    DataSet = cdsSetor
    Left = 105
    Top = 85
  end
  object dspSetor: TDataSetProviderUrb
    DataSet = sqlSetor
    Options = [poAllowCommandText]
    Left = 48
    Top = 85
  end
  object dspGrupo: TDataSetProviderUrb
    DataSet = sqlGrupo
    Options = [poAllowCommandText]
    Left = 48
    Top = 113
  end
  object dspModulo: TDataSetProviderUrb
    DataSet = sqlModulo
    Options = [poAllowCommandText]
    Left = 48
    Top = 141
  end
  object sqlSetor: TSQLDataSet
    DbxCommandType = 'Dbx.SQL'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = frmPrincipal.ConexaoBD
    Left = 20
    Top = 86
  end
  object sqlGrupo: TSQLDataSet
    DbxCommandType = 'Dbx.SQL'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = frmPrincipal.ConexaoBD
    Left = 20
    Top = 115
  end
  object sqlModulo: TSQLDataSet
    DbxCommandType = 'Dbx.SQL'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = frmPrincipal.ConexaoBD
    Left = 20
    Top = 142
  end
end
