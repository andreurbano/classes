...
begin
  ...
  begin
    ...
    // Andre ImageList.GetBitmap(Cedente.ContaBancaria.Banco.ImagemBanco,ABoleto.imgBanco1.Picture.Bitmap);
    if Cedente.ContaBancaria.Banco.Codigo = '033' then //Santander Banespa
      // Andre txtNumeroBanco1.Caption := Cedente.ContaBancaria.Banco.Codigo
    else
      // Andre txtNumeroBanco1.Caption := Cedente.ContaBancaria.Banco.Codigo + '-' + Cedente.ContaBancaria.Banco.Digito;
    // Andre txtLocalPagamento1.Caption := AnsiUpperCase(LocalPagamento);
    // Andre txtDataVencimento1.Caption := FormatDateTime('dd/mm/yyyy',DataVencimento);
    // Andre txtNomeCedente1.Caption := AnsiUpperCase(Cedente.Nome);
    // Andre txtCodigoCedente1.Caption := AAgenciaCodigoCedente;
    // Andre txtNumeroDocumento1.Caption := NumeroDocumento;
    // Andre txtDataProcessamento1.Caption := FormatDateTime('dd/mm/yyyy',Now);
    // Andre txtNossoNumero1.Caption := ANossoNumero;
    // Andre txtEspecie1.Caption := 'R$';
    // Andre txtValorDocumento1.Caption := FormatCurr('#,##0.00',ValorDocumento);
    txtNomeSacado1.Caption := AnsiUpperCase(Sacado.Nome);
    txtEnderecoSacado1.Caption := AnsiUpperCase(Sacado.Endereco.Rua + ', ' + Sacado.Endereco.Numero + '   ' + Sacado.Endereco.Complemento);
    txtCidadeSacado1.Caption := AnsiUpperCase(FormatarComMascara('00000-000;0; ',Sacado.Endereco.CEP) + '    ' + Sacado.Endereco.Bairro + '    ' + Sacado.Endereco.Cidade + '    ' + Sacado.Endereco.Estado);
    {Segunda via do boleto}
    ImageList.GetBitmap(Cedente.ContaBancaria.Banco.ImagemBanco,imgBanco2.Picture.Bitmap);
    // Andre txtNumeroBanco2.Caption := txtNumeroBanco1.Caption;
    txtNumeroBanco2.Caption := Cedente.ContaBancaria.Banco.Codigo + '-' + Cedente.ContaBancaria.Banco.Digito;
