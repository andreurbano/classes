...
type
  ...
  private
    ...
    procedure FTPWorkBegin(Sender: TObject; AWorkMode: TWorkMode;
      {$IFNDEF INDY_10} const {$ENDIF} AWorkCountMax: Integer);
    procedure FTPWork(Sender: TObject; AWorkMode: TWorkMode;
      {$IFNDEF INDY_10} const {$ENDIF} AWorkCount: Integer);
...
procedure Descompactar(Compactado: TFileName; DirDestino: string);
resourcestring
  EDecompressionErrorMessage = 'Arquivo %s corrompido';
var
  FileOutName, FileOutFullName: string;
...
var
  tempFile, NomeExe, batchName, NomeDos, TmpDir: string;
  lista: TStringList;
  existe: Boolean;
begin
  GetClientInstance;
...
var
  NomeExe, VersaoExecutavel, FileVer: string;
  sl: TStringList;
  i: Integer;
  b: Byte;
